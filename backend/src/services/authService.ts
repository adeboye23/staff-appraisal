...
export async function updateUserAccount(
  ...
  const existing = await query<UserRecord>("SELECT * FROM users WHERE id = $1", [id]);
  ...
  if (input.email && input.email !== user.email) {
    const emailTaken = await query<UserRecord>("SELECT * FROM users WHERE email = $1 AND id <> $2", [input.email, id]);
...
export async function loginUser(email: string, password: string) {
  const result = await query<UserRecord>("SELECT * FROM users WHERE email = $1", [email]);
