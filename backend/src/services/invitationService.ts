...
type InvitationCreateResult =
  | { skipped: true; email: string; reason: string }
  | {
      skipped: false;
...
function setupEmailHtml(input: { name: string; department: string; setupUrl: string }) {
  return `
...
export async function createInvitation(input: InviteInput): Promise<InvitationCreateResult> {
  const email = input.email.toLowerCase().trim();
  const department = await query<{ id: number; name: string }>("SELECT id, name FROM departments WHERE id = $1", [
    input.departmentId
  ]);
  ...
  const user = await query<{ id: number; name: string; email: string; role: Role }>(
    `
  ...
  const invitation = await query<{ id: number }>(
    `
...
export async function resendInvitation(invitationId: number, actorId: number) {
  const result = await query<{
    user_id: number;
    name: string;
    email: string;
